...
class WishlistCounterWidget extends StatefulWidget {
  final Function? onAmountChanged;
  final Function? onMelebihiBatas;
  final int qty;
  final double total;
  final int batasTotal;

  const WishlistCounterWidget({Key? key, this.onAmountChanged, this.onMelebihiBatas, required int this.qty, required double this.total, required int this.batasTotal}) : super(key: key);

  @override
  _WishlistCounterWidgetState createState() => _WishlistCounterWidgetState();
}

class _WishlistCounterWidgetState extends State<WishlistCounterWidget> {
  late int amount;
  ...
  @override
  void didUpdateWidget(covariant WishlistCounterWidget oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.total != oldWidget.total) {
      if (widget.total >= widget.batasTotal) {
        print("Total telah melebihi batas total");
      }
    }
  }
